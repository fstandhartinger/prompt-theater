...
docker exec "$DB" psql -U prompt -d prompt -c 'CREATE DATABASE prompt_test' >/dev/null 2>&1 || fail 'test database create'
docker run -d --name "$APP" --network "$NET" -p 3000:3000 -e DATABASE_URL=postgres://prompt:prompt@${DB}:5432/prompt -e FAL_FAKE=1 -e MODERATION_FAKE=1 -e SCENE_SECONDS=6 -e STRIPE_SECRET_KEY=sk_test_placeholder -e STRIPE_WEBHOOK_SECRET=whsec_e2e_secret -e PUBLIC_URL=http://127.0.0.1:3000 -e DATA_DIR=/data "$IMAGE" >/dev/null || fail 'app start'
...
# The unit suite needs a real Postgres: the money and recovery paths are tested against it.
docker run --rm --network "$NET" -e TEST_DATABASE_URL=postgres://prompt:prompt@${DB}:5432/prompt_test -e FAL_FAKE=1 -e MODERATION_FAKE=1 "$IMAGE" npm test || fail 'unit tests'
pass 'unit tests'
